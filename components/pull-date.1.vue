...
<script>
	export default {
		...
		methods:{
            ...
            checkDay (dateNum, outfocus) {//选择时间区间
				console.log(dateNum);
			...
			checkDate(dateNum){
				let check_date = this.year + '-' + this.month + '-' + dateNum;
				console.log(this.startEnv);
				if(!this.startEnv || this.startEnv == ''){
					this.startEnv = check_date;
					return false;
				}
				if(this.startEnv && (!this.endEnv || this.endEnv == '')){
					this.endEnv = check_date;
				}
				console.log(this.startEnv+'======'+this.endEnv)
			},
//             //两个日期之间间隔多少天
...
//             //比较两日期的大小
            ...
            showHalffun (dateNum, outfocus) {
				// console.log('showHalffun============'+dateNum+'=========='+outfocus);
                if (!outfocus) {
